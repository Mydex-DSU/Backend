...
//       // } else 
//       if (currentTime >= applicationStartTime){
//           // 상태 변경 로직
...
//                     //학생 테이블 노쇼 횟수 추가 + 1
//                     await db.query(
//                       'update student set stu_no_show_count = stu_no_show_count + 1 where stu_id = ?',
//                       [student.stu_id]
//                     )
          ...
          if (currentTime >= applicationStartTime && currentTime < applicationEndTime) {
            ...
            console.log("운영중")
              ...
              for (const student of studentProgram){
                ...
                if (student.stu_program_status === "참여완료"){ continue; }
                let fin_mydex_points = null;
                let temp = null;
                //1. 설문조사 참여 했나 안 했나를 먼저 보자
                if (studentcompletecheck[0].response_status_change_mydex_points === null)
                {
                  if (studentcompletecheck[0].stu_give_mydex_points !== null && parseInt(studentcompletecheck[0].stu_give_mydex_points) > 0)
                  {
                    ...
                    else {fin_mydex_points = 0;}
                  }
                  else if(studentcompletecheck[0].stu_give_mydex_points !== null && parseInt(studentcompletecheck[0].stu_give_mydex_points) < 0) 
                  {
                    if (studentcompletecheck[0].no_show_reason_response_status === 1) // 노쇼 설문조사 참여
                    {
                      if (studentcompletecheck[0].stu_give_mydex_points < -1){ // 최소 -1점은 받게 한다.
                        fin_mydex_points = studentcompletecheck[0].stu_give_mydex_points + 1;
                      }
                      else { fin_mydex_points = -1;}
                    }
                    else {
                      fin_mydex_points = studentcompletecheck[0].stu_give_mydex_points;
                      temp = studentcompletecheck[0].stu_give_mydex_points;
                    }
                  }
                  //fin_mydex_points가 양수 이면 노쇼가 아니고 음수이면 노쇼임.
                  if (fin_mydex_points !== null && studentcompletecheck[0].no_show_reason_response_status !== null && fin_mydex_points < 0)
                  {
                    console.log("fin_mydex_points가 양수 or 0 이면 노쇼가 아니고 음수이면 노쇼임.")
                    //학생 전체 노쇼 내역 업데이트 비교과에서는 노쇼가 1씩 더해지는 게 맞음.
                    await db.query(
                      'insert into student_noshow_history(stu_id, noshowhistory_recv_count, noshowhistory_reason_number) values(?,?,?)',
                      [student.stu_id, 1, program.program_id]
                  ...
                  //이제 학생 mydex 온도 포인트 부여하고 거래 내역
                  if (fin_mydex_points !== null)
                  {  //대출 포인트가 있냐 없냐 체크
                    console.log("대출 포인트가 있냐 없냐 체크")
                      const student_select = await db.query(
                        'select stu_name,stu_current_loan_points, stu_current_mydex_points from student where stu_id = ?',
                        [student.stu_id]
                      )
                      console.log(student_select[0].stu_name)
                      ...
                      if (student_select[0].stu_current_loan_points > 0) //대출 포인트 있음
                      {
                        ...
                        else if (fin_mydex_points < 0) // P < 0 //노쇼 했을 때 인데 대출 포인트가 있음
                        {
                          console.log("P < 0")
                          //대출 포인트 거래 내역 업데이트 ->
                          const insertloanpointtransactionhistory = await db.query(
                          ...
                          //student 테이블에서 학생 대출 포인트 업데이트 ->
                          await db.query(
                            'UPDATE student SET stu_current_loan_points = 0, stu_current_mydex_points = GREATEST(stu_current_mydex_points + ?, 1), stu_no_show_count = stu_no_show_count + 2 WHERE stu_id = ?;',
                            [fin_mydex_points, student.stu_id]
                            )
                          ...
                          if (status < 1) // M + P < 1 
                          {
                            ...
                            await db.query(
                              'insert into mydex_point_history(stu_id, mydexpointshistory_reason_name, mydexpointshistory_recv_count, mydexpointshistory_reason_number) values (?, ?, ?, ?)',
                              [student.stu_id, "비교과프로그램", (-student_select[0].stu_current_mydex_points) + 1, program.program_id]
                            )
                          }
                          else if (status >= 1)  // M + P >= 1 
                          {
                            console.log("M + P >= 1")
                            console.log(status)
                            await db.query(
                        ...
                        await db.query(
                          `UPDATE student_completes_program SET response_status_change_mydex_points = ? WHERE stu_id = ? and program_id = ?`,
                          [fin_mydex_points, student.stu_id, program.program_id]
                        );
                      }
                      else // 대출 포인트 없음
                      {
                        console.log(student_select[0].stu_name + " 대출 없음")
                        await db.query(
                          `UPDATE student 
                           SET stu_current_mydex_points = GREATEST(stu_current_mydex_points + ?, 1)
                           WHERE stu_id = ?`,
