...
router.post('/application', async (req, res) => {
    ...
    try 
    {
        ...
        if (loan_type_status === 1) // 전체 대출 
        {
            ...
            await req.db.query(
                'UPDATE student SET stu_current_loan_points = ? WHERE stu_id = ?;',
            ...
            await req.db.query(
                ...
                ,[stu_id, "전체 대출", loan_transaction_points, student[0].stu_current_loan_points]
            )
        ...
        else if (loan_type_status === 0) // 추가 대출
        {
            ...
            //학생 추가 대출 업데이트
            res.json({loan : student[0].stu_name + "의 추가 대출이 완료되었습니다."})
        }
    }
    catch(error)
    {
